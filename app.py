...
try:
    ...
    st.error("⚠️ CSV file not found. Please upload to your GitHub repo.")
    st.stop()
...
calc_scope = st.sidebar.radio("Normalization Scope", ["Regional (All Surrey)", "Local (Selected only)"])
...
with tab_dashboard:
    ...
    if app_mode == "1. Explore A Single Indicator":
        ...
        with col2:
            st.dataframe(map_df[['Area_Name', 'Value']].sort_values('Value', ascending=False), hide_index=True)
    ...
    # ==========================================
        ...
        st.info("This module will host pre-calculated, fixed corporate indices (e.g., The Surrey Index, Health and Wellbeing Strategy Index).")
        
        # Mocking an existing index dropdown for the prototype
        existing_index = st.selectbox("Select Existing Index", ["Surrey Index (Mock)", "Health and Wellbeing Strategy Index (Mock)"])
        st.write(f"Currently viewing the architecture for the **{existing_index}**. Once the final data is approved, it will be hard-coded here to prevent accidental alteration.")
    ...
    # ==========================================
        ...
        # Execute normalization and math (omitted detailed lines for brevity, uses your exact v5 logic)
        normalized_list = []
    ...
    # ==========================================
        ...
        st.write("Select two indicators to map their spatial distribution side-by-side.")
    ...
    # ==========================================
    # MODE 5: SUB-CATEGORY BREAKDOWN
    # ==========================================
    elif app_mode == "5. Sub-Category Breakdown":
        st.subheader("Sub-Category Composition")
        st.write("Understand the constituent parts of an aggregate indicator.")
        
        # Defensive check: Ensure the column exists before trying to chart it
        if 'Sub_Category' in df_raw.columns:
            cat_indicator = st.sidebar.selectbox("Select Aggregate Indicator", sorted(df_raw['Indicator_Name'].unique()))
            # (Stacked bar logic will go here once data is updated)
            st.success("Sub-Category data detected. Ready to plot.")
        else:
            st.warning("⚠️ Action Required: Data Format Update")
            st.write("""
            To use the Stacked Bar feature, your master CSV needs a new column called **`Sub_Category`**. 
            
            **Example for Qualifications:**
            * Indicator_Name: `Highest Qualification` | Sub_Category: `NVQ4+` | Value: `45`
            * Indicator_Name: `Highest Qualification` | Sub_Category: `NVQ3`  | Value: `20`
            """)
